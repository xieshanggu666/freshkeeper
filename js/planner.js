...
(function (global) {
  ...
  function buildCookingPlans(assessments, now, opts) {
    ...
    RECIPES.forEach(function (recipe) {
      var usedIds = {}, used = [], missing = [], frozenUsed = false;
      var score = 0, optionalFilled = 0, optionalTotal = 0;

      recipe.slots.forEach(function (slot) {
        if (slot.optional) optionalTotal++;
        var pick = pickForSlot(slot, assessments, usedIds, allowFrozen);
        if (!pick) {
          if (!slot.optional) {
            missing.push({ label: slot.label, groups: slot.groups });
          }
          return;
        }
        usedIds[keyOf(pick)] = true;
        used.push(pick);
      ...
      if (missing.length) return; // 必需槽位不齐，不出方案（缺料信息留给 UI 做“还差什么”）
